...
public class Chase : MonoBehaviour {
    ...
    void Update() {
        if (Vector3.Distance(player.position, this.transform.position) < visionDistance)
        {
            Vector3 direction = player.position - this.transform.position;
            float angle = Vector3.Angle(direction, this.transform.forward);
            if (angle < visionSpread)
            {
                RaycastHit hit;
                if (Physics.Raycast(this.transform.position, direction, out hit, visionSpread))
                {
                    if (hit.transform.gameObject.layer == 20 && playerBody.alive)
                    {
                        ...
                        else if (!attacking)
                        {
                            ...
                            InvokeRepeating("PrimaryAttack", .5f, 3f);
                        }
                    }
                }
            }
