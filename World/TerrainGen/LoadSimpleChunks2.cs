...
public class LoadSimpleChunks2 : MonoBehaviour
{
    ...
    void FindSimpleChunksToLoad()
    {
        ...
        WorldPos playerPos = new WorldPos(
            Mathf.FloorToInt(transform.position.x / SimpleChunk2.chunkSize) * SimpleChunk2.chunkSize,
            Mathf.FloorToInt(transform.position.y / SimpleChunk2.chunkSize) * SimpleChunk2.chunkSize,
            Mathf.FloorToInt(transform.position.z / SimpleChunk2.chunkSize) * SimpleChunk2.chunkSize
            );
        ...
        if (updateList.Count == 0)
        {
            ...
            for (int i = 0; i < chunkPositions.Length; i++)
            {
                ...
                WorldPos newSimpleChunkPos = new WorldPos(
                    chunkPositions[i].x * SimpleChunk2.chunkSize + playerPos.x,
                    chunkPositions[i].y * SimpleChunk2.chunkSize + playerPos.y,
                    chunkPositions[i].z * SimpleChunk2.chunkSize + playerPos.z
                    );

                //Get the chunk in the defined position
                SimpleChunk2 newSimpleChunk = simpleWorld.GetSimpleChunk(
                ...
                int player_y = (int)(Mathf.Floor(playerPos.y / SimpleChunk2.chunkSize));

                //load a column of chunks in this position
                for (int y = player_y - 3; y < player_y + 3; y++)
                {

                    for (int x = newSimpleChunkPos.x - SimpleChunk2.chunkSize; x <= newSimpleChunkPos.x + SimpleChunk2.chunkSize; x += SimpleChunk2.chunkSize)
                    {
                        for (int z = newSimpleChunkPos.z - SimpleChunk2.chunkSize; z <= newSimpleChunkPos.z + SimpleChunk2.chunkSize; z += SimpleChunk2.chunkSize)
                        {
                            buildList.Add(new WorldPos(
                                x, y * SimpleChunk2.chunkSize, z));
                        }
                    }
                    updateList.Add(new WorldPos(
                                newSimpleChunkPos.x, y * SimpleChunk2.chunkSize, newSimpleChunkPos.z));
                }
                return;
            }
        }
    ...
    void LoadAndRenderSimpleChunks()
    {
        ...
        if (updateList.Count != 0)
        {
            SimpleChunk2 chunk = simpleWorld.GetSimpleChunk(updateList[0].x, updateList[0].y, updateList[0].z);
            if (chunk != null)
                chunk.update = true;
